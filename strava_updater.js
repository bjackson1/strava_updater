...
var startTime = (((new Date).getTime() / 1000) - 180000000).toFixed(0);
    ...
    return https.get(
		...
		}, function(response)
		{
			...
			response.on('end', function()
            {
					...
					{
                        console.log(matchingActivities[i].id, matchingActivities[i].private, matchingActivities[i].name, matchingActivities[i].distance);
					}
				});
		});
}
